...
//1- Crud crud  = Crud(); // 2- بحط initial Binding (main / binding folder )
  ...
  ItemsData itemsData = ItemsData(Get.find()); // Get.find() كل شي محفون  List<CategoriesModel> data = [];
  ...
  late StatusRequest statusRequest;
     ...
     var response = await itemsData.getData(); //loading هون خلص 
  ...
  deleteItem(String id)async
  {
     ...
     var response = await itemsData.deleteData(id); //loading هون خلص // delete from db 
  ...
  {
    Get.toNamed(AppRoute.edititem, 
                           arguments: {
                            "itemsModels_" : itemsModel
                           });
