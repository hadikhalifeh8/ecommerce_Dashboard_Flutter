...
import 'package:ecommerce_dashboard/controller/items/editController.dart';
...
class ItemsEdit extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ItemEditController controler = Get.put(ItemEditController());


    return Scaffold(appBar: AppBar(centerTitle: true, title: const Text("Edit Item"),),

    body: GetBuilder<ItemEditController>(builder: (controller) => 
...
// if choose an image then show this image in ui
 ...
 RadioListTile(title: const Text("hide"),value: "0", groupValue: controler.active, onChanged: (val){controler.isActive("0");}),
 
 RadioListTile(title: const Text("active"), value: "1", groupValue: controler.active, onChanged: (val){controler.isActive("1");}),
       ...
       CustomButtonAuth(buttonText: "Update", onPressed: (){controler.edit(controller.itemsModel!.id.toString());})
